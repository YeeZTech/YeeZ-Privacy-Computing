...
bool simple_oram_sealed_file::get_block_id(const bytes &item_index_field_hash, uint32_t *block_id) { 
...
bool simple_oram_sealed_file::update_position_map(const uint8_t * position_map, uint32_t len) {
...
bool simple_oram_sealed_file::upload_path(uint32_t leaf, const uint8_t * encrpypted_path, uint32_t len) {
...
bool simple_oram_sealed_file::update_stash(const uint8_t * stash, uint32_t len) { 
...
bool simple_oram_sealed_file::update_merkle_hash(uint32_t leaf, const uint8_t * merkle_hash, uint32_t len) {
