...
namespace ypc {
namespace internal {
oram_sealed_file_base::oram_sealed_file_base(const std::string &file_path)
    : m_file(file_path) {}
...
bool simple_oram_sealed_file::download_oram_params(uint32_t *block_num,
...
bool simple_oram_sealed_file::get_block_id(bytes &item_index_field_hash,
                                           uint32_t *block_id) {
  return m_file.get_block_id(item_index_field_hash, block_id);
}

bool simple_oram_sealed_file::download_position_map(memref &posmap) {
  return m_file.download_position_map(posmap);
}

bool simple_oram_sealed_file::update_position_map(uint8_t *position_map,
                                                  uint32_t len) {
...
bool simple_oram_sealed_file::upload_path(uint32_t leaf,
                                          uint8_t *encrpypted_path,
                                          uint32_t len) {
  return m_file.upload_path(leaf, encrpypted_path, len);
}

bool simple_oram_sealed_file::download_stash(memref &st) {
  return m_file.download_stash(st);
}
...
bool simple_oram_sealed_file::download_merkle_hash(uint32_t leaf,
                                                   memref &merkle_hash) {
  return m_file.download_merkle_hash(leaf, merkle_hash);
}

bool simple_oram_sealed_file::update_merkle_hash(uint32_t leaf,
                                                 uint8_t *merkle_hash,
                                                 uint32_t len) {
