...
uint32_t keymgr_session::init_keymgr_session_oram() {
  if (keymgr_var::m_keymgr_session) {
    return stbox::stx_status::success;
  }

  keymgr_var::m_keymgr_session.reset(new stbox::dh_session_initiator(
      stbox::ocall_cast<uint32_t>(km_session_request_oram_ocall),
      stbox::ocall_cast<uint32_t>(km_exchange_report_oram_ocall),
      stbox::ocall_cast<uint32_t>(km_send_request_oram_ocall),
      stbox::ocall_cast<uint32_t>(km_end_session_oram_ocall)));
  keymgr_var::m_keymgr_session->set_verify_peer(km_verify_peer_enclave_trust);
