...
public:
  ...
  inline stbox::bytes do_parse(const stbox::bytes &param) {
    ...
    ypc::to_type<stbox::bytes, t_tax_item_t> converter(m_datasources[0].get());
    // param must be serialized ntpackage
    auto pkg = ypc::make_package<input_buf_t>::from_bytes(param);
    int counter = 0;
    hpda::processor::internal::filter_impl<t_tax_item_t> match(
        &converter, [&](const t_tax_item_t &v) {
          counter++;
    ...
    hpda::output::internal::memory_output_impl<t_tax_item_t> mo(&match);
    mo.get_engine()->run();
    ...
    typedef typename ypc::cast_obj_to_package<t_tax_item_t>::type package_t;
