...
sgx_status_t SGX_CDECL download_oram_params_OCALL(
    uint32_t *retval, const uint8_t *data_hash, uint32_t hash_size,
    uint32_t *block_num, uint32_t *bucket_num_N, uint8_t *level_num_L,
    uint32_t *bucket_str_size, uint32_t *batch_str_size);
sgx_status_t SGX_CDECL get_block_id_OCALL(
    uint32_t *retval, const uint8_t *data_hash, uint32_t hash_size,
    uint32_t *block_id, const uint8_t *param_hash, uint32_t param_hash_size);
sgx_status_t SGX_CDECL download_position_map_OCALL(uint32_t *retval,
                                                   const uint8_t *data_hash,
                                                   uint32_t hash_size,
                                                   uint8_t **position_map,
                                                   uint32_t *len);
sgx_status_t SGX_CDECL update_position_map_OCALL(uint32_t *retval,
                                                 const uint8_t *data_hash,
                                                 uint32_t hash_size,
                                                 uint8_t *position_map,
                                                 uint32_t len);
sgx_status_t SGX_CDECL download_path_OCALL(uint32_t *retval,
                                           const uint8_t *data_hash,
                                           uint32_t hash_size, uint32_t leaf,
                                           uint8_t **encrpypted_path,
                                           uint32_t *len);
sgx_status_t SGX_CDECL download_stash_OCALL(uint32_t *retval,
                                            const uint8_t *data_hash,
                                            uint32_t hash_size, uint8_t **stash,
                                            uint32_t *len);
sgx_status_t SGX_CDECL update_stash_OCALL(uint32_t *retval,
                                          const uint8_t *data_hash,
                                          uint32_t hash_size, uint8_t *stash,
                                          uint32_t len);
sgx_status_t SGX_CDECL upload_path_OCALL(uint32_t *retval,
                                         const uint8_t *data_hash,
                                         uint32_t hash_size, uint32_t leaf,
                                         uint8_t *encrpypted_path,
                                         uint32_t len);
sgx_status_t SGX_CDECL download_merkle_hash_OCALL(
    uint32_t *retval, const uint8_t *data_hash, uint32_t hash_size,
    uint32_t leaf, uint8_t **merkle_hash, uint32_t *len);
sgx_status_t SGX_CDECL update_merkle_hash_OCALL(
    uint32_t *retval, const uint8_t *data_hash, uint32_t hash_size,
    uint32_t leaf, uint8_t *merkle_hash, uint32_t len);
