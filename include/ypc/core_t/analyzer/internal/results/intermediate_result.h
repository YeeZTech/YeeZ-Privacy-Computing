...
public:
  uint32_t generate_result() {
    stbox::bytes algo_skey_b, user_skey_b, skey_sum_b, dian_key;
    typename skey_group_t::key_t algo_skey, user_skey, data_skey, skey_sum;

    LOG(INFO) << "request for user key";
    auto ret = keymgr_interface_t::request_private_key_for_public_key(
        m_user_pkey, user_skey_b, dian_key);
    ...
    // calcualte sum of skey generate tree
    LOG(INFO) << "calcualte sum of skey generate tree";
    ...
    add_ret = skey_group_t::add(skey_sum, skey_sum, user_skey);
