...
public:
  ...
  virtual bool get_block_id(const bytes &item_index_field_hash, uint32_t *block_id) = 0;

  virtual bool download_position_map(memref &posmap) = 0;

  virtual bool update_position_map(const uint8_t * position_map, uint32_t len) = 0;

  virtual bool download_path(uint32_t leaf, memref &en_path) = 0;

  virtual bool upload_path(uint32_t leaf, const uint8_t * encrpypted_path, uint32_t len) = 0;

  virtual bool download_stash(memref &st) = 0;

  virtual bool update_stash(const uint8_t * stash, uint32_t len) = 0;

  virtual bool read_root_hash(memref &root_hash) = 0;

  virtual bool download_merkle_hash(uint32_t leaf, memref &merkle_hash) = 0;

  virtual bool update_merkle_hash(uint32_t leaf, const uint8_t * merkle_hash, uint32_t len) = 0;
...
public:
  ...
  virtual bool update_position_map(const uint8_t * position_map, uint32_t len);
  virtual bool download_path(uint32_t leaf, memref &en_path);
  virtual bool upload_path(uint32_t leaf, const uint8_t * encrpypted_path, uint32_t len);
  virtual bool download_stash(memref &st);
  virtual bool update_stash(const uint8_t * stash, uint32_t len);
  ...
  virtual bool update_merkle_hash(uint32_t leaf, const uint8_t * merkle_hash, uint32_t len);
};
