 ...
 * \tparam IndexType type of index used to store the index position, usually
 * unsigned or size_t \tparam whether enabling the usage of aclist, this option
 * must be enabled manually
 */
template <typename IndexType, bool UseAcList = false> struct SparseCSRMBuilder {
private:
...
public:
  /*!
   * \brief step 1: initialize the number of rows in the data, not necessary
  ...
  /*!
   * \brief step 2: add budget to each rows, this function is called when aclist
   * is used \param row_id the id of the row \param nelem  number of element
   * budget add to this row
   */
   ...
   * element to each row, the number of calls shall be exactly same as
   * add_budget
   */
 ...
 * unsigned or size_t
 */
template <typename IndexType> struct SparseCSRMat {
private:
  /*! \brief pointer to each of the row */
  std::vector<size_t> rptr;
  /*! \brief index of nonzero entries in each row */
...
public:
  ...
  inline size_t NumRow(void) const { return rptr.size() - 1; }
  /*! \return number of elements r-th row */
  inline size_t NumElem(size_t r) const { return rptr[r + 1] - rptr[r]; }
  /*! \return r-th row */
