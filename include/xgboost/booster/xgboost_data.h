 ...
 *          feature matrix is going to be used by most inner loop of the
 * algorithm
 *
 * \tparam Derived type of actual implementation
 * \sa FMatrixS: most of time FMatrixS is sufficient, refer to it if you find it
 * confusing
 */
template <typename Derived> struct FMatrix {
public:
...
public:
  ...
  /*!
   * \brief get number of column groups, this ise used together with GetRow(
   * ridx, gid ) \return number of column group
   */
  inline unsigned NumColGroup(void) const { return 1; }
  /*!
  ...
  /*!
   * \brief get column backward iterator, starts from biggest fvalue, and
   * iterator back \param ridx column index \return reverse column iterator
   */
...
public:
  ...
  struct REntry {
    ...
    REntry(void) {}
    /*! \brief constructor */
    REntry(bst_uint findex, bst_float fvalue)
  ...
  struct Line {
    ...
    inline const REntry &operator[](unsigned i) const { return data_[i]; }
  };
  /*! \brief row iterator */
  struct RowIter {
    const REntry *dptr_, *end_;
    RowIter(const REntry *dptr, const REntry *end) : dptr_(dptr), end_(end) {}
    inline bool Next(void) {
      if (dptr_ == end_)
        return false;
      else {
    ...
    inline bst_uint findex(void) const { return dptr_->findex; }
    inline bst_float fvalue(void) const { return dptr_->fvalue; }
  };
  /*! \brief column iterator */
  struct ColIter : public RowIter {
    ColIter(const REntry *dptr, const REntry *end) : RowIter(dptr, end) {}
    inline bst_uint rindex(void) const { return this->findex(); }
  };
  /*! \brief reverse column iterator */
  struct ColBackIter : public ColIter {
    ColBackIter(const REntry *dptr, const REntry *end) : ColIter(dptr, end) {}
...
public:
  ...
  /*!  \brief get number of rows */
  inline size_t NumRow(void) const { return row_ptr_.size() - 1; }
  /*!
   * \brief get number of nonzero entries
   * \return number of nonzero entries
   */
  inline size_t NumEntry(void) const { return row_data_.size(); }
  ...
  inline size_t AddRow(const std::vector<bst_uint> &findex,
    ...
    for (size_t i = 0; i < findex.size(); i++) {
      if (findex[i] < fstart || findex[i] >= fend)
        continue;
  ...
  inline RowIter GetRow(size_t ridx) const {
    ...
    return RowIter(&row_data_[row_ptr_[ridx]] - 1,
                   &row_data_[row_ptr_[ridx + 1]] - 1);
  }
  /*!  \brief get row iterator*/
  inline RowIter GetRow(size_t ridx, unsigned gid) const {
