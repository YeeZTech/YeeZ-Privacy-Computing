...
def create_oram_file(oram_sealed_data_url):
    cmd = "touch {}".format(oram_sealed_data_url)
    output = common.execute_cmd(cmd)
    return [cmd, output]

class classic_job_convert:
    def __init__(self, crypto, name, data_url, convert_parser_url, convert_parser2_url, oram_parser_url, plugin_url, con_read_num, config={}):
        self.crypto = crypto
        self.name = name
        self.data_url = data_url
        self.convert_parser_url = convert_parser_url
        self.convert_parser2_url = convert_parser2_url
        self.oram_parser_url = oram_parser_url
    ...
    def run(self):
        ...
        oram_sealed_data_url = sealed_data_url + ".oram"
        summary = {}
        ...
        if self.read_count == 0:
            ...
            self.data_hash = job_step.read_data_hash(sealed_output)
            summary['data-hash'] = self.data_hash
            print("done seal data with hash: {}, cmd: {}".format(self.data_hash, r[0]))
            create_oram_file(oram_sealed_data_url)
        ...
        if self.read_count == 0:
            self.enclave_hash = job_step.read_parser_hash(self.convert_parser_url)
        if self.read_count == 1:
            self.enclave_hash = job_step.read_parser_hash(self.convert_parser2_url)
        if self.read_count > 1:
            self.enclave_hash = job_step.read_parser_hash(self.oram_parser_url)
        ...
        if self.read_count <= 1:
            param_json = job_step.generate_request(
                self.crypto, self.input, data_key_file, param_output_url, self.config)
        else:
            param_json = job_step.generate_request(
        ...
        # 5. call fid_convert_analyzer 转换文件格式
        if self.read_count == 0:
            input_obj = {
                "input_data_url": sealed_data_url,
                "input_data_hash": self.data_hash,
                "shu_info": {
            ...
            result_json = job_step.fid_convert_analyzer(self.shukey_json, self.data_shukey_json, rq_forward_json, self.enclave_hash, input_data,
                                                self.convert_parser_url, pkey, {}, self.crypto, param_json, [], parser_input_file, parser_output_file)
        ...
        if self.read_count == 1:
            ...
            parser_output_file = self.name + "parser_output.json"
            result_json = job_step.fid_convert_analyzer(self.shukey_json, self.data_shukey_json, rq_forward_json, self.enclave_hash, input_data,
                                                self.convert_parser2_url, pkey, {}, self.crypto, param_json, [], parser_input_file, parser_output_file)
        ...
        # 6. call fid_oram_analyzer 使用ORAM格式的加密文件进行分析任务
        if self.read_count > 1:
            root_hash_url = self.name + ".root_hash"
            r = job_step.get_root_hash(oram_sealed_data_url, root_hash_url)
            root_hash = r[1]

            input_obj = {
                "input_data_url": oram_sealed_data_url,
            ...
            result_json = job_step.fid_oram_analyzer(self.shukey_json, rq_forward_json, self.enclave_hash, input_data,
                                                self.oram_parser_url, pkey, {}, self.crypto, param_json, [], parser_input_file, parser_output_file)
            ...
            self.all_outputs.append(parser_input_file)
            self.all_outputs.append(parser_output_file)
            self.all_outputs.append(root_hash_url)
