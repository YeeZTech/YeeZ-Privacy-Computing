...
onchain_data_reader::get_data_with_hash(const ypc::bytes &hash) {
...
void dummy_onchain_data_reader::init() {
  ...
  try {
    ...
    BOOST_FOREACH (boost::property_tree::ptree::value_type &data,
      ...
      auto dhash = data.second.get<ypc::bytes>("data-hash");
      auto pkey = data.second.get<ypc::bytes>("provider-pkey");
