...
uint32_t iris_parser_module::begin_parse_data_item() {
  auto retval = ecall<uint32_t>(::begin_parse_data_item);
  return retval;
}

uint32_t iris_parser_module::parse_data_item(const uint8_t *data, size_t len) {
  auto retval = ecall<uint32_t>(::parse_data_item, data, (len));
  return retval;
}

uint32_t iris_parser_module::end_parse_data_item() {
  auto retval = ecall<uint32_t>(::end_parse_data_item);
  return retval;
}

uint32_t iris_parser_module::get_enclave_hash(ypc::bytes &_enclave_hash) {
  uint32_t hash_size;
...
uint32_t iris_parser_module::get_analyze_result(ypc::bytes &result) {
  uint32_t res_size;
  uint8_t *res;
...
uint32_t iris_parser_module::init_data_source(const ypc::bytes &info) {
  return ecall<uint32_t>(::init_data_source, info.data(), info.size());
}
uint32_t iris_parser_module::init_model(const ypc::bytes &info) {
  return ecall<uint32_t>(::init_model, info.data(), info.size());
}
uint32_t iris_parser_module::get_parser_type() {
...
extern "C" parser_module_interface *create_instance(const char *mod_path) {
  return new iris_parser_module(mod_path);
}
