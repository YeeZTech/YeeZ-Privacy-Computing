...
uint32_t download_oram_params_OCALL(const uint8_t *data_hash,
                                    uint32_t hash_size, uint32_t *block_num,
...
uint32_t get_block_id_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                            uint32_t *block_id, const uint8_t *param_hash,
                            uint32_t param_hash_size);
uint32_t download_position_map_OCALL(const uint8_t *data_hash,
                                     uint32_t hash_size, uint8_t **position_map,
                                     uint32_t *len);
...
uint32_t download_path_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                             uint32_t leaf, uint8_t **encrpypted_path,
                             uint32_t *len);
...
uint32_t upload_path_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                           uint32_t leaf, uint8_t *encrpypted_path,
                           uint32_t len);
uint32_t download_merkle_hash_OCALL(const uint8_t *data_hash,
                                    uint32_t hash_size, uint32_t leaf,
                                    uint8_t **merkle_hash, uint32_t *len);
uint32_t update_merkle_hash_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                                  uint32_t leaf, uint8_t *merkle_hash,
                                  uint32_t len);
}
...
uint32_t get_block_id_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                            uint32_t *block_id, const uint8_t *param_hash,
                            uint32_t param_hash_size) {
  return o_parser->get_block_id_OCALL(data_hash, hash_size, block_id,
                                      param_hash, param_hash_size);
}

uint32_t download_position_map_OCALL(const uint8_t *data_hash,
                                     uint32_t hash_size, uint8_t **position_map,
                                     uint32_t *len) {
  return o_parser->download_position_map_OCALL(data_hash, hash_size,
                                               position_map, len);
}

uint32_t update_position_map_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                                   uint8_t *position_map, uint32_t len) {
  return o_parser->update_position_map_OCALL(data_hash, hash_size, position_map,
                                             len);
}
...
uint32_t download_path_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                             uint32_t leaf, uint8_t **encrpypted_path,
                             uint32_t *len) {
  return o_parser->download_path_OCALL(data_hash, hash_size, leaf,
                                       encrpypted_path, len);
}
...
uint32_t upload_path_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                           uint32_t leaf, uint8_t *encrpypted_path,
                           uint32_t len) {
  return o_parser->upload_path_OCALL(data_hash, hash_size, leaf,
                                     encrpypted_path, len);
}
...
uint32_t download_merkle_hash_OCALL(const uint8_t *data_hash,
                                    uint32_t hash_size, uint32_t leaf,
                                    uint8_t **merkle_hash, uint32_t *len) {
  return o_parser->download_merkle_hash_OCALL(data_hash, hash_size, leaf,
                                              merkle_hash, len);
}
...
uint32_t update_merkle_hash_OCALL(const uint8_t *data_hash, uint32_t hash_size,
                                  uint32_t leaf, uint8_t *merkle_hash,
                                  uint32_t len) {
  return o_parser->update_merkle_hash_OCALL(data_hash, hash_size, leaf,
                                            merkle_hash, len);
}
