...
// txt reader
define_nt(txt_batch, std::string);
                     ...
                     const std::string &data_file)
{
    ...
    ifs.close();
    std::ofstream re_ofs(data_file, std::ios::out);
    ...
    while (0u != item_number--)
    {
        ...
        if (ret)
        {
            ...
            // txt reader
            for(auto &batch_data_item : batch_data) {
              auto datap = ypc::make_package<package_t>::from_bytes(batch_data_item);
              auto item = datap.get<::txt_batch>();
              re_ofs << item;
            }
        }
    }
...
int main(int argc, char *argv[])
{
    ...
    {
        std::cerr << "Invalid directory path: " << sealed_data_dir << std::endl;
        return -1;
    }
    ...
    {
        uint32_t status = futures[i].get();
        if (status != 0u)
        {
            std::cout << "Failed to unseal file " << filePath[i].filename().string() << "\n";
            return -1;
        }
    }
